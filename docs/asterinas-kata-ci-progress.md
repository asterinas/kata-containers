...
- Read `requirements.md` and confirmed three requested deliverables:
  1. Import the CI plus all referenced scripts from `jjf-dev/asterinas` PR `#57` into this repo.
...
- Inspected `jjf-dev/asterinas#57` with `gh`.
...
- Imported `.github/workflows/test-asterinas-kata.yml` and the full `tools/kata/` helper directory from `jjf-dev/asterinas#57`.
