...
def run_kernel_developer_scenario(args: argparse.Namespace) -> pathlib.Path:
    ...
    try:
        ...
        announce("[kernel-developer] building local kernel with make kernel BOOT_METHOD=qemu-direct INITRAMFS=off")
        shell.run("cd /root/asterinas && make kernel BOOT_METHOD=qemu-direct INITRAMFS=off", timeout=7200)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay the documented Kata flows with Asterinas as the guest kernel with pexpect")
    parser.add_argument(
        "--scenario",
